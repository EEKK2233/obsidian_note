...
1、在t1目录删除main.cpp和CMakeLists.txt外的所有内容，然后建立build 目录
...
```
生成件中，有两个可执行文件：
1. 被重新命名的helloExe.exe（在build\bin目录中）
2. 新增的测试文件helloTest.exe（在build\tests目录中）
