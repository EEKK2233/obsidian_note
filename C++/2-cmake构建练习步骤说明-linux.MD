...
5、生成hello，make利用makefile，完成编译和链接，t1目录中有hello

```shell
make
```

6、执行hello，输出main.cpp的字符串
...
1、在t1目录删除main.cpp和CMakeLimkdirsts.txt外的所有内容，然后建立build 目录
...
2、在t1的build目录下，执行cmake -G"Unix Makefiles" .. 
...
3、在t1的build目录下，执行make，可在在build目录下看到hello文件
...
4、编辑src/main.cpp ,增加函数调用，修改为如下代码
...
void FuncLib()
{
    cout << "You are enter lib func!" << endl;
}
```
...
ADD_LIBRARY(funclib STATIC ${LIBHELLO_SRC})//静态库
```

用于cmake产生makefile文件

步骤4：编辑t1目录下的CMakeLists.txt
```
PROJECT (HELLO)
ADD_SUBDIRECTORY(src bin)
...
步骤5：进入build目录，删除目录下所有内容，然后执行如下命令进行外部编译
...
--- 在build\lib目录看到libfundlib.a 静态库
...
使用外部共享静态库和头文件
步骤1：在t1/lib目录下创建include目录，且将t1/lib目录下funclib.hpp文件移动到t1/lib/include目录下
...
步骤1：添加子目录tests：新建文件tests\main.cpp，添加代码如下
...
步骤2：在tests目录中，新建源文件test.cpp，内容如下
...
步骤3：在t1/include目录中，新建头文件sut.hpp，代码如下
...
步骤6：创建t1\tests\CMakeLists.txt ，配置测试如下：
...
步骤7：把1-必备知识与环境准备/4-实践代码start工程/cpp-training-start.zip压缩包中的cpp-training/tests/googletest拷贝到tests文件夹下
...
```
生成件中，有两个可执行文件：
1. 被重新命名的helloExe（在build\bin目录中）
2. 新增的测试文件helloTest（在build\tests目录中）
分别对应CMakeLists.txt中使用ADD_EXECUTABLE配置的可执行文件。
...
void FuncLib()
{
    std::cout << "You are enter func dynamic lib!" << std::endl;
}
```
...
```shell
cp build/lib/libfunclib.dll build/bin
```
